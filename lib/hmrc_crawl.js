...
function getPage(url, fn) {
  // TODO cache invalidation so that we get new months that are added
  if (cache[url]) {
    //console.info('[INFO] hmrc_crawl#getPage - cache hit for url:', url)
    return fn(null, cache[url])
  }
  jsdom.env(url, [
    'http://code.jquery.com/jquery-1.7.1.min.js'
  ], function(err, window) {
    if (!err) cache[url] = window
    fn(err, window)
  })
}
...
function getValuesForYear(year, fn) {
  ...
  function monthsHandler(month) {
    ...
    getEuroValueforMonth(month.href, function(value) {
      month.rate = value
      if (!--counter) fn(null, months)
    })
  }
